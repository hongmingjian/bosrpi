...
{
  emmc_reg_t *emmc = (emmc_reg_t *)(MMIO_BASE_PA+EMMC_REG);
...
static void gpioSetFunction(int32_t pin, uint32_t val)
{
...
static void gpioSetPull(int32_t pin, uint32_t val)
{
	gpio_reg_t *gpio = (gpio_reg_t *)(MMIO_BASE_PA+GPIO_REG);
	uint32_t enbit;
...
{
	if(SD_OK == sdTransferBlocks( sector*512, count, buffer, 0 ))
		return 0;
	return -1;
}
...
uint32_t DFS_WriteSector(uint8_t unit, uint8_t *buffer,
        uint32_t sector, uint32_t count)
{
	if(SD_OK == sdTransferBlocks( sector*512, count, buffer, 1 ))
		return 0;
	return -1;
}
